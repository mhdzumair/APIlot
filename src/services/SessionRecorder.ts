...
export interface AllSessionsExport {
  sessions: Session[];
...
export class SessionRecorder {
  ...
  startRecording(name: string | null = null): Session | null {
    if (this.isRecording) {
  ...
  stopRecording(): Session | null {
    ...
    console.log(
      '[RECORDER] Stopped recording session:',
      completedSession.id,
      'Total requests:',
      completedSession.requests.length
    );
    return completedSession;
  }

  // Record a request
  recordRequest(requestData: IncomingRequestData): RecordedRequest | false {
    ...
    console.log(
      '[RECORDER] Recorded request:',
      entry.id,
      entry.operationName ?? entry.endpoint
    );
    return entry;
  }
  ...
  recordResponse(
    requestId: string,
    responseData: IncomingResponseData
  ): RecordedRequest | false {
    if (!this.isRecording || !this.currentSession) {
      return false;
    }
    ...
    entry.response = responseData.response ?? null;
    entry.responseStatus = responseData.status ?? null;
    entry.responseStatusText = responseData.statusText;
    entry.responseHeaders = responseData.headers ?? null;
    entry.responseTime =
  ...
  // Export a session to a serialisable object
  ...
  // Import a session from a previously exported object
  importSession(data: SessionExport): Session {
    if (!data.session) {
  ...
  getPlaybackPosition(): PlaybackPosition | null {
    ...
    return {
      ...
      currentRequest:
        this.playbackState.session.requests[this.playbackState.currentIndex] ?? null,
    };
  }
  ...
  resumePlayback(): boolean {
    ...
    console.log(
      '[RECORDER] Playback resumed from index:',
  ...
  stepForward(): PlaybackPosition | null {
    if (!this.playbackState) {
      return null;
    }
    ...
    if (
      this.playbackState.currentIndex <
      this.playbackState.session.requests.length - 1
    ) {
      this.playbackState.currentIndex++;
  ...
  stepBackward(): PlaybackPosition | null {
    if (!this.playbackState) {
      return null;
    }
  ...
  // Seek to specific request index
  seekTo(index: number): PlaybackPosition | null {
    if (!this.playbackState) {
      return null;
    }
  ...
  modifyResponse(requestId: string, newResponse: unknown): boolean {
    if (!this.playbackState) {
      return false;
    }
    ...
    const request = this.playbackState.session.requests.find(
      (r) => r.id === requestId
    );
    if (request) {
  ...
  // Get response for a request during playback (modifications take priority)
  getPlaybackResponse(requestId: string): PlaybackResponse | null {
    if (!this.playbackState) {
      return null;
    }
  ...
  // Find a matching recorded response for an incoming live request
  findMatchingRecordedResponse(requestData: IncomingRequestData): PlaybackResponse | null {
    if (!this.playbackState) {
      return null;
    }
    ...
    const matchingRequest = session.requests.find((r) => {
      if (
        requestData.requestType === 'graphql' &&
        r.requestType === 'graphql'
      ) {
        return (
          r.operationName === requestData.operationName && r.url === requestData.url
        );
      } else if (
        requestData.requestType === 'rest' &&
        r.requestType === 'rest'
      ) {
  ...
  importAll(data: Partial<AllSessionsExport>): void {
    if (data.sessions) {
